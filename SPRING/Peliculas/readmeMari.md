...
--  Aceleran las queries de JOIN FETCH que harás en los repositorios para listar actores de una película y películas de un actor.
...
public class Movie {
    ...
    // TODO: añadir la relación @OneToMany hacia MovieCast
    // Pista: cascade = ALL, orphanRemoval = true, Set en lugar de List
}
```
...
public class Actor {
    ...
    // TODO: añadir la relación inversa @OneToMany hacia MovieCast
    // Pista: sin CascadeType.ALL, solo PERSIST y MERGE
}
```
...
public class MovieCastId implements Serializable {
    ...
    // TODO: declarar los dos campos de la PK compuesta
    // Pista: movieId y actorId, ambos Long
...
public class MovieCast {
    ...
    // TODO: @EmbeddedId con MovieCastId

    // TODO: @MapsId("movieId") → @ManyToOne hacia Movie
    // TODO: @MapsId("actorId") → @ManyToOne hacia Actor

    // TODO: atributos propios de la relación:
    //   - characterName (String, nullable = false)
    //   - screenMinutes (int, nullable = false)
    //   - salaryOverride (BigDecimal, nullable)
    //   - active (boolean, nullable = false)

    // Obligatorio: implementar equals() y hashCode() basados en el id embebido
}
```
...
- [ ] `MovieCastId` implementa `Serializable` con `equals`/`hashCode` correctos
- [ ] `MovieCast` implementa `equals`/`hashCode` basados en el `id` embebido
- [ ] `Movie` tiene métodos helper `addActor` / `removeActor`
- [ ] El repositorio usa `JOIN FETCH` para evitar el problema N+1
- [ ] `DELETE /movies/{id}` borra también sus filas en `movie_cast` (cascade)
- [ ] `DELETE /movies/{id}/cast/{actorId}` NO borra el actor ni la película
- [ ] La prueba 4 devuelve `409` y no `500`
