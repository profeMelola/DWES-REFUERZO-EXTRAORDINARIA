...
public class Distributor {
    ...
    // TODO: @ManyToOne hacia Country (FetchType.LAZY)
...
public class Release {
    ...
    // TODO: @ManyToOne hacia Movie (FetchType.LAZY)
    // TODO: @ManyToOne hacia Distributor (FetchType.LAZY)
    ...
    // TODO: @OneToMany hacia BoxOfficeEntry (cascade = ALL, orphanRemoval = true)
}
```
...
public class BoxOfficeEntry {
    ...
    // TODO: @ManyToOne hacia Release (FetchType.LAZY)
    // TODO: @ManyToOne hacia Country (FetchType.LAZY)
...
```java
@Query("""
    SELECT new es.daw.clinicaapi.dto.report.TopGrossingMovieReport(
        m.id,
        m.title,
        m.genre,
        count(b),
    ...
    FROM BoxOfficeEntry b
        JOIN b.release r
        JOIN r.movie m
        JOIN r.distributor d
    WHERE b.active = true
...
public class ReportController {
    ...
    public List<TopGrossingMovieReport> topGrossing(
        ...
        return boxOfficeRepository.topGrossingMovies(genre, from, to, pageable);
    }
}
```
...
-- DISTRIBUTORS
INSERT INTO distributors (name, country_id, active) VALUES
    ('Warner Bros USA',       1, true),
    ('Warner Bros España',    2, true),
    ('Universal France',      3, true),
    ('Sony Pictures UK',      4, true);

-- RELEASES (usando movies del data.sql de Fase I)
-- Inception (1), The Dark Knight (2), Interstellar (3)
INSERT INTO releases (movie_id, distributor_id, release_date, active) VALUES
    (1, 1, '2010-07-16', true),   -- Inception - Warner USA
    (1, 2, '2010-08-20', true),   -- Inception - Warner España
    (2, 1, '2008-07-18', true),   -- The Dark Knight - Warner USA
    (2, 4, '2008-08-01', true),   -- The Dark Knight - Sony UK
    (3, 1, '2014-11-07', true),   -- Interstellar - Warner USA
    (3, 3, '2014-11-12', true);   -- Interstellar - Universal France

-- BOX_OFFICE_ENTRIES
-- release_id | country_id | period_start | period_end | gross       | screens
INSERT INTO box_office_entries (release_id, country_id, period_start, period_end, gross, screens, active) VALUES
--  Inception USA (release 1)
...
- [ ] Las 4 entidades nuevas creadas con las relaciones correctas
- [ ] `schema.sql` actualizado con las 4 tablas nuevas
- [ ] `data.sql` actualizado con los datos de taquilla
- [ ] `TopGrossingMovieReport` implementado como `record`
- [ ] Query JPQL cruza mínimo 4 tablas con `JOIN`
- [ ] Todos los filtros son opcionales (`:param IS NULL OR`)
- [ ] El endpoint acepta `page` y `size` como parámetros
- [ ] Sin filtros, el orden es descendente por `totalGross`
